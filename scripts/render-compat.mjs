...
MATRIX.forEach((_, i) => {
  if (i % 2 === 1) out.push(`<rect class="band" y="${HEAD_H + i * ROW_H}" width="${width - PAD * 2}" height="${ROW_H}"/>`);
});
out.push('</g>');
...
AGENTS.forEach((name, j) => {
  ...
  out.push(`<text class="agent" x="${cx}" y="${HEAD_H / 2 + 4.5}" text-anchor="middle">${esc(name)}</text>`);
});
...
out.push(`<text class="legend" x="${width - PAD * 2 - 16}" y="${ly + 4}" text-anchor="end">Numbers refer to the notes below.</text>`);

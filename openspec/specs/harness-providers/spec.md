...
While it is open, every shared watch helper SHALL subscribe when the file interface offers `onWatchChurn`, and SHALL unsubscribe when it closes. After churn a helper SHALL re-verify once what it covers and service only what differs from what it last serviced:
...
The re-verification SHALL go through the helper's debounce, so a burst of opens and closes causes one pass, and the pass happens after the churn rather than during it. It SHALL be caused only by churn, never by a clock: while no watch is opening or closing, no timer exists. An entry that did not change SHALL NOT be reported and SHALL NOT be read. A helper's own open counts as churn.
...
- **THEN** each open helper re-verifies once

#### Scenario: Nothing changed
- **WHEN** churn happens and nothing a helper covers has changed
- **THEN** it reports nothing, and no timer remains once the pass has run
